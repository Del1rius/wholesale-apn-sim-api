...
def dashboard_view(request):
    ...
    for sim in sim_cards:
        ...
        usage_percentage = 0
        if sim.data_limit_mb and sim.data_limit_mb > 0:
            usage_percentage = (float(sim_total_usage) / sim.data_limit_mb) * 100
...
def sim_list_view(request):
    ...
    for sim in sim_cards:
        ...
        print(f"DEBUG: SIM {sim.iccid} - Status: {sim.status} -> {status_display_map.get(sim.status, sim.status)}")
...
def sim_detail_view(request, iccid):
    ...
    if not (user.is_superuser or user.role == 'network_admin'):
        if sim_obj.organization != user.organization:
            messages.error(request, 'You do not have permission to view this SIM card')
            return redirect('dashboard')
        ...
        try:
            ...
            if new_limit <= 0:
                ...
                if float(total_usage) >= new_limit:
                    ...
                    if sim_obj.status != 'suspended':
                        sim_obj.status = 'suspended'
                        messages.warning(request, f'Data limit updated to {new_limit} MB. SIM has been suspended due to exceeding the limit.')
                    else:
                        messages.success(request, f'Data limit updated to {new_limit} MB successfully!')
                else:
                    # Usage is below new limit - ensure SIM is active if it was suspended
                    if sim_obj.status == 'suspended':
                        sim_obj.status = 'assigned'
                        messages.success(request, f'Data limit updated to {new_limit} MB. SIM has been reactivated.')
                    else:
                        messages.success(request, f'Data limit updated to {new_limit} MB successfully!')
    ...
    class FormattedSIM:
        def __init__(self, sim_obj, total_usage, usage_percentage, remaining_mb):
            ...
            self.OrgID = type('obj', (object,), {'Name': sim_obj.organization.name if sim_obj.organization else 'N/A'})()
